...
const COUNT_WORDS = ['واحد','اثنان','ثلاثة','أربعة','خمسة','ستة','سبعة','ثمانية','تسعة'];
...
function speak(txt) {
  return new Promise((resolve, reject) => {
    ...
    if (fs.existsSync(outPath)) {
      console.log(`  skip (exists): ${hash}.mp3  "${txt}"`);
...
(async () => {
  ...
  console.log('\nGenerated count words audio.');

  // Patch __NARR_AUDIO__ in ch2 lesson 3
  const L3 = path.join(ROOT, 'Interactive book for elementary school chapter 2', 'moosa-chp 2-lesson 3.html');
  let src = fs.readFileSync(L3, 'utf8');
  ...
  if (start !== -1) {
    ...
    if (objEnd !== -1) {
      ...
      fs.writeFileSync(L3, src);
      console.log('Patched __NARR_AUDIO__ in moosa-chp 2-lesson 3.html');
    }
  }

  console.log('\nAdditions:');
  Object.entries(additions).forEach(([k, v]) => console.log(`  "${k}" → ${v}`));
  console.log('Done.');
})();
